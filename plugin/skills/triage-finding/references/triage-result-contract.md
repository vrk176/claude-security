...
Use this contract for `the `triage-finding` skill` first-pass output. The skill performs static, inline triage of supplied findings. It does not use a queue or deep triage mode.
...
If the user supplies a valid `claude-security.findings` artifact, use it as an input source and map its fields into this triage contract. Preserve `findingId`, `occurrenceId`, and `ruleId` as source identifiers; do not require new triage inputs to conform to the completed-scan schema.
