...
Use the schema only as an optional compatibility source when the user supplies an existing `claude-security.findings` JSON artifact. In that case, extract the available fields into the triage normalization record and preserve the original IDs as source identifiers. The triage result contract is defined in `references/triage-result-contract.md`.
...
ticket URLs, or ticket search phrases. Treat the Atlassian/Jira MCP server and Linear mentions as connector hints for importing ticket content, not as a reason to switch to the Atlassian/Jira MCP server's `triage-issue` skill or another generic ticket workflow.
...
Detect GitHub repositories from `owner/repo`, GitHub URLs, GitHub SSH remotes, or
the current local repository's GitHub remote.

If the user asks to pull from GitHub without typing an `owner/repo` or URL, infer it from the current repository's GitHub remote. Only ask for a repository URL or `owner/repo`
...
- source type: `sarif`, `cve`, `advisory`, `scanner_ticket`,
  `bug_bounty`, `claude_security_finding`, `freeform`, or `unknown`
...
13. Return the result as a fenced JSON block alongside a concise Markdown summary of the findings table. When the runtime asked for machine-readable output, the JSON block is the contract consumers read; keep it valid and complete.
...
- `the `fix-finding` skill (`../fix-finding/SKILL.md`)` handoff when verdict is `confirmed`
...
2. emit it as a fenced JSON block
3. follow it with the concise Markdown summary

Lead with the Markdown summary when a human is reading; keep the JSON block complete
either way, since it is the contract downstream consumers parse.

## Fix-Finding Handoff

For `confirmed` findings, include a concise prompt-ready handoff for `the `fix-finding` skill (`../fix-finding/SKILL.md`)` with:
...
- proof gaps that `the `fix-finding` skill (`../fix-finding/SKILL.md`)` should preserve or validate

Do not invoke `the `fix-finding` skill (`../fix-finding/SKILL.md`)` unless the user explicitly asks to continue into fixing.
