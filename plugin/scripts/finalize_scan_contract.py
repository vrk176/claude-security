...
FINGERPRINT_ALGORITHM = "claude-security/v1"
SARIF_SCHEMA = "https://docs.oasis-open.org/sarif/sarif/v2.1.0/os/schemas/sarif-schema-2.1.0.json"
# SARIF presentation identity. These are display/property names only: nothing in the
# scan contract or its schemas constrains them, and _validate_sarif does not check the
# fingerprint key name. They are what a SARIF consumer (e.g. GitHub code scanning)
# shows as the producing tool, so they carry this port's branding rather than the
# upstream contract identity in FINGERPRINT_ALGORITHM.
SARIF_TOOL_NAME = "Claude Security"
SARIF_FINGERPRINT_KEY = "claudeSecurity/v1"
SARIF_PROPERTY_PREFIX = "claudeSecurity"
...
SAFE_SCHEMA_PATTERNS = {
    r"^(?![^:/?#]+://[^/?#]*@)[^?#]+$",
    r"^claude-security-snapshot/v1:sha256:[a-f0-9]{64}$",
    r"^(?!/)(?!.*(?:^|/)\.\.(?:/|$))(?!.*\\).+$",
    r"^[a-f0-9]{64}$",
    r"^(?!.*(?:^|/)\.\.(?:/|$))(?!.*\\)artifacts/.+$",
    r"^csf_[a-f0-9]{24}$",
    r"^occ_[a-f0-9]{24}$",
    r"^[a-z0-9][a-z0-9._/-]*$",
    r"^claude-security/v1:sha256:[a-f0-9]{64}$",
...
def _generate_report_projection(
    ...
    spec = importlib.util.spec_from_file_location("claude_security_report_projection", script)
...
def _windows_scan_local_files() -> Any:
    ...
    if _WINDOWS_SCAN_LOCAL_FILES is None:
        ...
        spec = importlib.util.spec_from_file_location("claude_security_windows_scan_files", script)
        if spec is None or spec.loader is None:
...
def _populate_unsealed_manifest_envelope(
    ...
    manifest["documentType"] = "claude-security.scan-manifest"
    manifest["schemaVersion"] = SCHEMA_VERSION
    scan["status"] = "completed"
    scan["coverageRef"] = "coverage.json"
    scan["findingsRef"] = "findings.json"
    if completion_binding is None:
        started_at = os.environ.get("CLAUDE_SECURITY_STARTED_AT")
        if started_at is not None:
            _validate_date_time(started_at, "CLAUDE_SECURITY_STARTED_AT")
...
def _populate_unsealed_artifact_envelope(
    ...
    findings["documentType"] = "claude-security.findings"
    findings["schemaVersion"] = SCHEMA_VERSION
    coverage["documentType"] = "claude-security.coverage"
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("documentType") != "claude-security.scan-manifest":
        raise ContractError("manifest.documentType: expected claude-security.scan-manifest")
...
def _validate_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    if findings.get("documentType") != "claude-security.findings":
        raise ContractError("findings.documentType: expected claude-security.findings")
...
def _sarif_result(
    ...
    partial_fingerprints = {
        SARIF_FINGERPRINT_KEY: finding["fingerprints"]["primary"],
    }
...
def build_sarif(
    ...
    run: dict[str, Any] = {
        ...
        "properties": {
            f"{SARIF_PROPERTY_PREFIX}SchemaVersion": manifest["schemaVersion"],
            f"{SARIF_PROPERTY_PREFIX}TargetKind": target["kind"],
        },
    }
...
def _write_sarif_projection_if_possible(
    ...
    try:
        ...
        print(
            f"claude-security: warning: automatic SARIF export failed: {error}. "
            "Run `claude-security export <scan-dir> --export-format sarif` to retry.",
            file=sys.stderr,
        )
