...
def state_dir() -> Path:
    state_dir = os.environ.get("CLAUDE_SECURITY_STATE_DIR")
    if state_dir:
        return Path(state_dir).expanduser().resolve()
    claude_home = Path(os.environ.get("CLAUDE_HOME", "~/.claude")).expanduser()
    return (claude_home / "state" / "plugins" / "claude-security").resolve()
...
def resolve_scan_id(connection: sqlite3.Connection, scan_id: str) -> str:
    try:
        ...
        if not matches:
            raise SystemExit("Scan not found.") from None
        if len(matches) > 1:
            raise SystemExit(
    ...
    try:
        ...
        if workspace["default_mode"] == "deep" and workspace["thread_id"] is not None:
            ...
            if owned_active_scan is not None:
                raise SystemExit(
                    "This session already has an active Deep Scan for the selected "
...
def cancel_scan(connection: sqlite3.Connection, args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if thread_id is not None and owning_thread_id != thread_id:
            raise SystemExit("A scan can only be canceled from its owning session.")
...
def require_reviewed_patch_applied(
    ...
    with tempfile.TemporaryDirectory(prefix="claude-security-remediation-") as temporary:
