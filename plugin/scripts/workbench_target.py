...
def worktree_content_digest_for_context(
    ...
    *,
    ...
    update_digest_field(digest, b"format", b"claude-security-snapshot/v1")
    ...
    return f"claude-security-snapshot/v1:sha256:{digest.hexdigest()}"
...
def directory_content_digest(target: Path, *, excluded: tuple[Path, ...] = ()) -> str:
    ...
    digest = hashlib.sha256()
    update_digest_field(digest, b"format", b"claude-security-directory/v1")
    for path in paths:
